...
LOGO_PATH = ROOT / "output" / "myongmoon_modu" / "eum_logo_gold.png"
...
GREY = (170, 180, 200)
MUTED = (110, 125, 150)
WATERMARK = (240, 243, 248)  # 옅은 회색 워터마크 (흰 배경용)
INK = (28, 36, 56)
SOFT_LINE = (220, 225, 232)
...
def add_background(img: ImageDraw.ImageDraw):
    ...
    for _ in range(90):
        ...
        r = rnd.choice([1, 1, 2, 2])
        draw_dot(draw, x, y, r, soft)
    return draw
...
def make_chapter_page(idx: int, part_roman: str, part_name: str, eng_label: str) -> Image.Image:
    ...
    # 가는 라인
    line_y = main_y + 320
    draw.line([(margin_x, line_y), (W - margin_x, line_y)], fill=SOFT_LINE, width=2)
    ...
    text_left(draw, (margin_x, title_y), part_name, f_title, INK)
    ...
    for x in range(margin_x, W - margin_x + 1, 28):
        draw_dot(draw, x, foot_dot_y, 3, SOFT_LINE)
...
def make_solution_divider() -> Image.Image:
    ...
    text_left(draw, (bullet_x + 80, main_y), "및 해설", f_main, INK)
    ...
    text_left(draw, (margin_x, copy_y), "틀린 문제는 다시 풀어보고,", f_copy, INK)
    text_left(draw, (margin_x, copy_y + 110), "맞힌 문제도 풀이를 점검하세요.", f_copy, INK)
...
CHAPTERS = [
    ("I", "삼각함수의 그래프~수열의 귀납적정의", "TRIG GRAPHS ~ RECURSIVE SEQUENCES"),
]
...
def main():
    ...
    names = ["ch1_perm_comb_matrix", "solution"]
