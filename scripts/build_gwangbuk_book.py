...
SRC_PDF = Path("/Users/youngwoolee/Downloads/260603_광북 1-1-b 프린트대비_원본.pdf")
OUT_DIR = ROOT / "output" / "gwangbuk_1_1_b"
OUT_DIR.mkdir(parents=True, exist_ok=True)

COVER = OUT_DIR / "cover.pdf"
DIVIDERS = ROOT / "output" / "summit_point" / "dividers.pdf"
PROBLEMS_PDF = OUT_DIR / "problems_2col.pdf"
OUT_PDF = OUT_DIR / "광북_1-1-b_SUMMIT_POINT.pdf"

N_PROBLEMS = 30
PART_NO = "PART 1"
CHAPTER_SHORT = "여러가지 방정식·부등식"

# 빠른정답 (원본 빠른정답표에서 전사 — A·01~A·30, 원본 문항순 동일)
ANSWERS = [
    "⑤", "①", "⑤", "①", "⑤", "②", "③", "①", "16", "③",
    "②", "②", "②", "5", "18", "21", "⑤", "②", "①", "①",
    "21", "11", "①", "③", "②", "⑤", "21", "10", "②", "①",
...
def main():
    ...
    out.insert_pdf(div, from_page=3, to_page=3)        # 4) 정답·해설 디바이더

    qa = build_quick_answer_page()                     # 5) 빠른정답 (SUMMIT 스타일)
    out.insert_pdf(qa)
    qa.close()
