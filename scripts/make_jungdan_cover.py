"""중단원 평가(2-2단원) 교재 표지 PDF.

SUMMIT POINT 표지의 다크 네이비 + 골드 톤을 동일하게 사용.
타이틀만 'TEXTBOOK EVAL' / '중단원 평가 정복'.
"""
...
def make_cover() -> Image.Image:
    ...
    # 메인 제목: TEXTBOOK (작게, 위) / POINT (크게, 아래 골드)
    title_cx = W // 2
    line1_cy = 760
    f_main = font(260, "paper_black")
    text_centered(draw, (title_cx + 6, line1_cy + 8), "TEXTBOOK",
                  f_main, (8, 14, 30))
    text_centered(draw, (title_cx, line1_cy), "TEXTBOOK", f_main, WHITE)
    ...
    # 부제 배지
    badge_y = line2_cy + 270
    f_badge = font(78, "cafe24")
    text_centered(draw, (W // 2, badge_y), "대수 1학기 기말대비", f_badge, GOLD_BRIGHT)
    bb_w = 420
    ...
    text_centered(draw, (W // 2, subtitle_y), "단원평가 정복", f_sub, WHITE)

    f_sub_en = font(40, "aggro")
    text_centered(draw, (W // 2, subtitle_y + 120),
                  "C O N Q U E R   T H E   T E X T B O O K", f_sub_en, GREY)
    ...
    text_left(draw, (220, foot_y), "Textbook Mastery Workbook", f_foot, GOLD)
    f_foot_kr = font(40, "cafe24")
    text_left(draw, (220, foot_y + 80), "교재의 정점, 한 권에 정복", f_foot_kr, GREY)
