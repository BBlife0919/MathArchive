"""KERNEL POINT 챕터 디바이더 — 2 챕터 + 정답해설.

중단원정복(TEXTBOOK POINT) 디바이더와 동일 다크 네이비 + 골드 톤.
"""
...
def fit_text_size(draw, text, family, max_w, start=280, floor=110):
...
def make_chapter_page(idx, roman, part_name, eng_label):
    ...
    text_right(draw, (W - margin_x, top_y - 26), "KERNEL POINT", f_label, GREY)
...
from extract_kernel_problems import CHAPTERS  # noqa: E402
...
def main():
    ...
    print(f"[OK] {OUT_PDF} ({len(pages)} pages)")
