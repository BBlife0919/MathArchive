"""직선의 방정식 WORKBOOK 재빌드 — 통번호(1부터), 매쏠로지 100문제."""
...
rows_raw = json.load(open('/tmp/jikseon_workbook.json'))

# 매쏠로지 R2 이미지 URL 인지 확인 (has_image + question_text 안 <<IMG:>>?)
# 원본 워크북 이미 이미지 embed 돼있으니 그대로 사용
rows = []
for i, q in enumerate(rows_raw, 1):
    ...
    # chapter 전부 하나로 통일 (분리 안 하도록)
    q['chapter'] = "직선의 방정식 · WORKBOOK"
...
pdf = generate_book_pdf(
    rows, title="직선의 방정식", subtitle="공통수학2 WORKBOOK",
    include_source=True, overrides=overrides, logo_path=None,
    divider_meta_top="공통수학2 · 직선의 방정식 · WORKBOOK",
    divider_footer_title="공통수학2 · 직선의 방정식 · WORKBOOK",
    ...
    cover_footer_sub="공통수학2 · 직선의 방정식 · WORKBOOK",
    page_running_left="공통수학2 직선의 방정식 · WORKBOOK",
    extra_css=CROME_CSS, extra_js=tpl.TYPESET_JS,
    running_numbering=True,
)
open('/tmp/jikseon_workbook.pdf','wb').write(pdf)
