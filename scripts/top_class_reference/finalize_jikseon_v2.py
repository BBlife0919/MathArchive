...
KP_SRC = "/tmp/jikseon_kernel_v2.pdf"
WB_SRC = "/tmp/jikseon_workbook.pdf"
OUT = "/Users/youngwoolee/Downloads/수업자료/탑반 교재/직선의방정식_KERNEL+WORKBOOK_합본.pdf"
...
<div class="title-block">
  ...
  <div class="rule"></div>
  <div class="sub">공통수학2_직선의 방정식</div>
</div>
...
# 즉 KP 안 페이지 idx k → 합본 페이지 = k + 2 (1-based)
    ...
    for i in range(sol_start, kp.page_count):
        ...
        for m in re.finditer(r"(?<![\d])(\d{1,3})\s*번\b", txt):
            ...
            if 1 <= n <= 200 and n not in sol_map:
                sol_map[n] = i + 1 + COMBINED_OFFSET  # 합본 기준 페이지

# ── 4. 각 문제 페이지에 "정답 및 해설 → p.N" 오버레이 (우측 상단)
overlay_count = 0
for pidx in range(kp.page_count):
    ...
    text = f"정답  p.{sol_pg}"
    # 머리말 우측 끝 (헤더 텍스트와 같은 baseline). 헤더 y 대략 30~40.
...
# 합본 재배치: 표지(1) + 목차(2) + KP표지(3) + KP본문(4~) + WB
kp_first_page_final = 3  # KP 표지가 합본 3p
kp_sol_page_final = (sol_start + 1) + COMBINED_OFFSET if sol_start is not None else kp_first_page_final
wb_cover_final = COMBINED_OFFSET + kp_total + 1  # KP 끝 다음이 WB 시작
...
<div class="sub">공통수학2 · 직선의 방정식 · KERNEL POINT + WORKBOOK</div>
<hr>
<ul>
